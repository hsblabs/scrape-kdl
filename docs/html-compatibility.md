...
The versioned manifest is `fixtures/html-compat/manifest.json`. It pins parser mode, decoded encoding, input bytes, selector observations, text, deterministic inner HTML, attributes, normalization, source identity, and approved divergences. A difference inside this portable surface fails pull-request verification.

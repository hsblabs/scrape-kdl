...
- Preflighted input declarations for unique names, supported types, required/default exclusion, and every default literal even when a provided value overrides it. Duplicate declarations no longer resolve by last-write-wins, and hidden malformed defaults fail before URL expansion or source acquisition; executor statement coverage remains 86.7%.
- Preflighted source kind, session-policy enums, and URL-template segment discriminators after preserving fetch-mode and JavaScript security-gate ordering. Typed segments with invalid `kind` values and unknown segment implementations now fail before input resolution or source acquisition; executor statement coverage increased to 86.8%.
- Made malformed array/nullable type formatting panic-free and preflighted field successful/effective types plus JavaScript return types. Nil nested type metadata now produces deterministic `E_IR_INVALID` errors before source acquisition instead of risking a nil-pointer panic during diagnostics; executor statement coverage increased to 86.9%.
- Preflighted declared transform and transform-call input/output type metadata after preserving target and signature diagnostic precedence. Empty or malformed type references now fail deterministically before transform execution or source acquisition; executor statement coverage remains 86.9%.
- Preflighted declared pipeline and field transform-chain continuity. Pipelines must contain a call, every call input must equal the preceding output, and final values must be assignable to the declared transform or field type before source acquisition; executor statement coverage remains 86.9%.
- Preflighted root and nested output-object plus field/collection `kind` discriminators. Concrete Go values with schema-incompatible discriminator strings no longer bypass IR constants before HTTP transport or browser acquisition; executor statement coverage increased to 87.0%.
- Preflighted pipeline, match, external transform, built-in target, and declared target `kind` discriminators while preserving external-registration and target-resolution ordering for valid IR. Concrete values with invalid schema constants now fail before source acquisition; executor statement coverage remains 87.0%.
- Preflighted text, HTML, attribute, and JavaScript value-source `kind` discriminators plus every browser workflow-step discriminator. Schema-incompatible concrete Go values now fail before HTTP transport or browser acquisition; executor statement coverage remains 87.0%.
- Enforced `string` raw types for text, HTML, and attribute value sources, matching the compiler and the runtime's concrete DOM read results. Hand-built IR can no longer disguise string reads as another type and bypass transform continuity; executor statement coverage increased to 87.1%.
- Preflighted declared-transform call input/output metadata against the referenced declaration. Inputs must satisfy the declaration and outputs must match it exactly, preventing internally continuous but dishonest call metadata from reaching source acquisition; executor statement coverage increased to 87.3%.
- Enforced the normative scalar/nullable-scalar boundary and duplicate-case prohibition for match transforms during runtime preflight. Malformed hand-built match IR now fails before source acquisition instead of executing semantics unavailable to v0.1 source programs; executor statement coverage remains 87.3%.
- Preflighted top-level extractor kind, IR version, language version, and positive document version after preserving existing fetch-mode and JavaScript opt-in ordering. Unsupported metadata no longer reaches HTTP transport or browser acquisition; executor statement coverage increased to 87.4%.
- Normalized match-case inputs to their declared scalar type before runtime equality checks. Numeric cases such as `1.0` now match equivalent normalized values, and differently spelled but equivalent cases are rejected as duplicates during preflight; executor statement coverage increased to 87.6%.
- Rejected empty external-transform symbols before registry lookup, matching the source grammar and compiler's non-empty requirement. An empty host-registry key can no longer make malformed external-transform IR executable; executor statement coverage remains 87.6%.
- Rejected empty input declaration and URL-template input-segment names, and fixed deterministic unknown-input detection so an empty provided name is no longer mistaken for the sentinel. All paths now fail before transport; executor statement coverage remains 87.6%.
...
- `487aa29` docs: record duplicate transform rejection
- `8c3ac3a` fix: reject duplicate output identities
- `ddeda71` docs: record output identity preflight
...
- `f49aca4` docs: record JavaScript source preflight
- `731dc47` fix: preflight input declarations
- `f5c4bf7` docs: record input declaration preflight
- `19007ff` fix: preflight source structure
- `dcb23d1` docs: record source structure preflight
- `df056f6` fix: preflight field type metadata
- `301273a` docs: record field type preflight
- `5bac7ae` fix: preflight transform type metadata
- `20885ea` docs: record transform type preflight
- `89d91fa` fix: preflight transform type continuity
- `3d823eb` docs: record transform continuity preflight
- `55445a8` fix: preflight output kind discriminators
- `9bd7d93` docs: record output discriminator preflight
- `ced9f78` fix: preflight transform kind discriminators
- `84d29ed` docs: record transform discriminator preflight
- `851258b` fix: preflight runtime kind discriminators
- `ed611bb` docs: record runtime discriminator preflight
- `2d33cc3` fix: preflight DOM value source types
- `7f1e29d` docs: record DOM source type preflight
- `d2b1ff8` fix: preflight declared transform call types
- `1c2325c` docs: record declared call type preflight
- `9ef1047` fix: preflight match transform structure
- `4f7815c` docs: record match structure preflight
- `3638891` fix: preflight extractor metadata
- `08fd4c1` docs: record extractor metadata preflight
- `697bf11` fix: normalize match case comparisons
- `4574af9` docs: record match comparison normalization
- `73fb727` fix: reject empty external transform symbols
- `38480d4` docs: record external symbol preflight
- `d3bb2b7` fix: reject empty runtime input names
- `17b0937` docs: record runtime input name hardening

...
1. Configure the `npm-publish` GitHub Environment with required reviewers and an `NPM_TOKEN` secret that can publish the `@hsblabs` scope.
2. Configure the `github-pages` Environment with required reviewers and GitHub Pages using GitHub Actions as its source.
3. Confirm ownership of both npm package names.
4. Decide when to make the repository public. Do not publish a Go module tag while the repository is private.

The npm and Pages workflows also require typed confirmations. Environment approval remains the primary protection against accidental publication.
...
6. Manually run `Publish npm packages` for the same version. Prereleases use the `next` dist-tag; stable releases use `latest`. The workflow publishes core first and confirms registry visibility before publishing the Playwright adapter.
...
Also install the Go and npm modules into new consumer projects, execute the documented basic example, download every CLI archive, verify `checksums.txt`, and run the native archive on Linux amd64, Linux arm64, macOS amd64, and macOS arm64.

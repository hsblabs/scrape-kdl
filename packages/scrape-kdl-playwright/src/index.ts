...
export type { BrowserAdapter, BrowserAdapterLease, BrowserElement } from "@hsblabs/scrape-kdl";
...
export class PlaywrightAdapter implements BrowserAdapter, BrowserAdapterLease {
      ...
      await this.#guard(
        async () => {
          ...
          if (generation !== this.#contextGeneration) {
            await context.close();
            throw new Error("navigation context was invalidated");
          }
          this.#context = context;
          context.setDefaultTimeout(this.#defaultTimeoutMs);
          context.setDefaultNavigationTimeout(this.#defaultTimeoutMs);
          if (options.session?.cookies !== undefined)
            await context.addCookies(cookieParameters(url, options.session.cookies));
    ...
    await this.#guard(
      () =>
        this.#requiredPage()
          .locator(selector)
          .first()
          .waitFor({ state, timeout: playwrightTimeout(options.timeoutMs) }),
      options.timeoutMs,
      options.signal,
    );
  }
    ...
    await this.#guard(
      () =>
        this.#requiredPage()
          .locator(selector)
          .first()
          .click({ timeout: playwrightTimeout(options.timeoutMs) }),
      options.timeoutMs,
      options.signal,
    );
  }
    ...
    await this.#guard(
      () =>
        this.#requiredPage()
          .locator(selector)
          .first()
          .fill(value, { timeout: playwrightTimeout(options.timeoutMs) }),
      options.timeoutMs,
      options.signal,
    );
  }
    ...
    await this.#guard(
      () =>
        this.#requiredPage()
          .locator(selector)
          .first()
          .press(key, { timeout: playwrightTimeout(options.timeoutMs) }),
      options.timeoutMs,
      options.signal,
    );
  }
        ...
        ? () =>
            this.#requiredPage().evaluate(async (script) => {
              ...
              if (typeof callable !== "function")
                throw new Error("JavaScript source must evaluate to a callable function");
    ...
    return this.#guard(
      async () => {
        const locators = await (
          scope === undefined ? this.#requiredPage().locator(selector) : unwrap(scope).locator(selector)
        ).all();
  ...
  async queryLimit(
    ...
    return this.#guard(
      async () => {
        const locator = scope === undefined ? this.#requiredPage().locator(selector) : unwrap(scope).locator(selector);
        const count = Math.min(await locator.count(), Math.max(0, limit));
        return Array.from({ length: count }, (_, index) => new PlaywrightElement(locator.nth(index)));
      },
      this.#defaultTimeoutMs,
      options.signal,
    );
  ...
  async text(element: BrowserElement, options: Pick<BrowserOperationOptions, "signal">): Promise<string> {
    return this.#guard(
      () => unwrap(element).evaluate((node) => node.textContent ?? ""),
      this.#defaultTimeoutMs,
      options.signal,
    );
  }
  ...
  async html(element: BrowserElement, options: Pick<BrowserOperationOptions, "signal">): Promise<string> {
    return this.#guard(
      () => unwrap(element).evaluate((node) => node.innerHTML),
      this.#defaultTimeoutMs,
      options.signal,
    );
  }
  ...
  async #guard<T>(
    ...
    try {
      return await Promise.race([operation(), guarded]);
    } catch (error) {
  ...
  #assertOpen(): void {
    if (this.#closed) throw new Error("Playwright adapter is closed");
  }
...
function unwrap(element: BrowserElement): Locator {
  if (!(element instanceof PlaywrightElement))
    throw new Error("browser element is not owned by this Playwright adapter");
...
function playwrightTimeout(value: number): number {
  return Math.min(positiveTimeout(value), MAX_TIMER_MS);
}
...
function delay(milliseconds: number): Promise<void> {
  return new Promise((resolve) => {
    scheduleTimeout(milliseconds, resolve);
  });
}

...
import type {
  CollectionIR, ExtractorIR, FieldIR, JsonScalar, JsonValue, MatchTransformIR, OutputObjectIR,
  ResolvedTransformCallIR, TransformIR, TypeRef,
} from "./ir.js";
...
function resolveInputs(ir: ExtractorIR, provided: Readonly<Record<string, JsonValue>>): Map<string, JsonScalar> {
  ...
  for (const input of ir.inputs) {
    ...
    if (value === undefined) {
      if (input.required) throw new ExecutionError("E_INPUT_REQUIRED", `required input ${JSON.stringify(input.name)} is missing`, { path: `input.${input.name}` });
      continue;
    }
    if (!inputMatches(value, input.type)) throw new ExecutionError("E_INPUT_TYPE", `input ${JSON.stringify(input.name)} is not assignable to ${input.type}`, { path: `input.${input.name}` });
...
function expandTemplate(ir: ExtractorIR, inputs: ReadonlyMap<string, JsonScalar>): URL {
  ...
  for (const segment of ir.source.fetch.urlTemplate.segments) {
    if (segment.kind === "literal") output += segment.value;
      ...
      if (value === undefined) throw new ExecutionError("E_INPUT_REQUIRED", `URL template input ${JSON.stringify(segment.name)} is missing`, { path: `input.${segment.name}` });
      output += percentEncode(scalarString(value));
    }
  }
  try {
    const url = new URL(output);
    if (url.protocol !== "http:" && url.protocol !== "https:" || url.host === "") throw new Error("unsupported target");
    return url;
  } catch (error) { throw new ExecutionError("E_URL_INVALID", `expanded URL is not an absolute HTTP(S) URL: ${JSON.stringify(output)}`, { cause: error }); }
}
...
function scalarString(value: JsonScalar): string {
  ...
  if (typeof value === "number" && !Number.isFinite(value)) throw new ExecutionError("E_INPUT_TYPE", "float must be finite");
  return String(value);
}

function percentEncode(value: string): string {
  return [...new TextEncoder().encode(value)].map((byte) =>
    byte >= 0x61 && byte <= 0x7a || byte >= 0x41 && byte <= 0x5a || byte >= 0x30 && byte <= 0x39 || [0x2d, 0x2e, 0x5f, 0x7e].includes(byte)
...
async function fetchHTML(initialURL: URL, options: ExecutionOptions): Promise<string> {
  ...
  const timeoutMs = options.requestTimeoutMs !== undefined && options.requestTimeoutMs > 0 ? options.requestTimeoutMs : DEFAULT_TIMEOUT_MS;
  const maxBytes = options.maxResponseBytes !== undefined && options.maxResponseBytes > 0 ? options.maxResponseBytes : DEFAULT_MAX_RESPONSE_BYTES;
  ...
  try {
    for (let redirect = 0; redirect <= MAX_REDIRECTS; redirect++) {
      ...
      try { response = await fetchImplementation(url, { method: "GET", headers, redirect: "manual", signal: controller.signal }); }
      catch (error) {
        if (timedOut) throw new ExecutionError("E_TIMEOUT", "request timed out", { cause: error });
        if (options.signal?.aborted === true) throw new ExecutionError("E_EXECUTION_CANCELED", errorMessage(options.signal.reason), { cause: error });
        throw new ExecutionError("E_HTTP_FETCH", errorMessage(error), { cause: error });
      }
      if (isRedirect(response.status)) {
        const location = response.headers.get("location");
        await response.body?.cancel();
        if (location === null) throw new ExecutionError("E_HTTP_STATUS", `redirect status ${response.status} has no Location header`);
        if (redirect === MAX_REDIRECTS) throw new ExecutionError("E_HTTP_FETCH", `stopped after ${MAX_REDIRECTS} redirects`);
        let next: URL;
        try {
          next = new URL(location, url);
          if (next.protocol !== "http:" && next.protocol !== "https:" || next.host === "") throw new Error("unsupported redirect target");
        } catch (error) { throw new ExecutionError("E_HTTP_FETCH", `invalid redirect target ${JSON.stringify(location)}`, { cause: error }); }
      ...
      if (!response.ok) { await response.body?.cancel(); throw new ExecutionError("E_HTTP_STATUS", `unexpected HTTP status ${response.status} ${response.statusText}`.trim()); }
...
function applySessionCookies(headers: Headers, session: Session | undefined, url: URL, retainHeader: boolean): void {
  ...
  for (const cookie of session?.cookies ?? []) if (cookieApplies(cookie, url, retainHeader)) values.push(`${cookie.name}=${cookie.value}`);
...
async function readBoundedBody(response: Response, maximum: number, signal: AbortSignal, timedOut: () => boolean, parent: AbortSignal | undefined): Promise<Uint8Array> {
  ...
  try {
    for (;;) {
      ...
      try { result = await reader.read(); }
        ...
        if (timedOut() || signal.reason?.name === "TimeoutError") throw new ExecutionError("E_TIMEOUT", "request timed out", { cause: error });
        if (parent?.aborted === true) throw new ExecutionError("E_EXECUTION_CANCELED", errorMessage(parent.reason), { cause: error });
        throw new ExecutionError("E_HTTP_READ", errorMessage(error), { cause: error });
      }
      if (result.done) break;
      size += result.value.byteLength;
      if (size > maximum) { await reader.cancel(); throw new ExecutionError("E_HTTP_BODY_TOO_LARGE", `response exceeds ${maximum} bytes`); }
...
function decodeHTML(bytes: Uint8Array, contentType: string): string {
  ...
  if (startsWith(bytes, [0xef, 0xbb, 0xbf])) { bytes = bytes.slice(3); charset = "utf-8"; }
  else if (startsWith(bytes, [0xff, 0xfe])) { bytes = bytes.slice(2); charset = "utf-16le"; }
  else if (startsWith(bytes, [0xfe, 0xff])) { bytes = bytes.slice(2); charset = "utf-16be"; }
  charset = normalizeCharset(charset);
  if (charset === "us-ascii" && bytes.some((byte) => byte > 0x7f)) throw new ExecutionError("E_HTML_DECODE", "response is not valid US-ASCII");
  ...
  try { return new TextDecoder(charset, { fatal: true }).decode(bytes); }
    ...
    if (error instanceof RangeError) throw new ExecutionError("E_HTML_CHARSET_UNSUPPORTED", `unsupported HTML charset ${JSON.stringify(charset)}`, { cause: error });
...
function sniffMetaCharset(bytes: Uint8Array): string | undefined {
  ...
  const match = /<meta\s+[^>]*(?:charset\s*=\s*["']?\s*([^\s"'/>;]+)|content\s*=\s*["'][^"']*charset\s*=\s*([^\s"'/>;]+))/iu.exec(source);
  return match?.slice(1).find((value) => value !== undefined);
}
...
const WINDOWS_1252 = [0x20ac, 0x0081, 0x201a, 0x0192, 0x201e, 0x2026, 0x2020, 0x2021, 0x02c6, 0x2030, 0x0160, 0x2039, 0x0152, 0x008d, 0x017d, 0x008f, 0x0090, 0x2018, 0x2019, 0x201c, 0x201d, 0x2022, 0x2013, 0x2014, 0x02dc, 0x2122, 0x0161, 0x203a, 0x0153, 0x009d, 0x017e, 0x0178];
function decodeWindows1252(bytes: Uint8Array): string { return Array.from(bytes, (byte) => String.fromCodePoint(byte >= 0x80 && byte <= 0x9f ? WINDOWS_1252[byte - 0x80]! : byte)).join(""); }
function startsWith(bytes: Uint8Array, prefix: readonly number[]): boolean { return prefix.every((byte, index) => bytes[index] === byte); }

async function executeDocument(state: RuntimeState, document: DocumentNode): Promise<ExtractionResult> {
  const value = await executeObject(state, document, state.ir.output, "output");
...
async function executeObject(state: RuntimeState, scope: DocumentNode | ElementNode, object: OutputObjectIR, path: string): Promise<Record<string, JsonValue>> {
...
async function executeField(state: RuntimeState, scope: DocumentNode | ElementNode, field: FieldIR, path: string): Promise<JsonValue> {
  let value: JsonValue;
  try { value = readFieldValue(scope, field, path); }
  catch (error) {
...
function readFieldValue(scope: DocumentNode | ElementNode, field: FieldIR, path: string): JsonValue {
  let selected: ElementNode | undefined;
  if (field.selection !== undefined) {
    const matches = queryAll(scope, field.selection.selector);
  ...
  if (field.valueSource.kind === "text") {
    if (selected === undefined) throw new ExecutionError("E_IR_INVALID", "text value source has no selected element", { path });
    return textContent(selected);
  }
  if (field.valueSource.kind === "html") {
    if (selected === undefined) throw new ExecutionError("E_IR_INVALID", "HTML value source has no selected element", { path });
    return innerHTML(selected);
  }
  if (field.valueSource.kind === "attribute") {
    if (selected === undefined) throw new ExecutionError("E_IR_INVALID", "attribute value source has no selected element", { path });
...
function recoverField(state: RuntimeState, field: FieldIR, path: string, cause: unknown): JsonValue {
...
async function executeCollection(state: RuntimeState, scope: DocumentNode | ElementNode, collection: CollectionIR, path: string): Promise<readonly JsonValue[]> {
  const rows = queryAll(scope, collection.selector);
  const output: JsonValue[] = [];
  for (let index = 0; index < rows.length; index++) {
    checkCanceled(state, `${path}[${index}]`);
...
async function applyCalls(state: RuntimeState, input: JsonValue, calls: readonly ResolvedTransformCallIR[], path: string): Promise<JsonValue> {
  ...
  for (const call of calls) {
    checkCanceled(state, path);
    try {
      value = call.target.kind === "builtin" ? applyBuiltin(call.target.name, value, call) : await applyDeclared(state, call.target.symbolId, value, path);
      if (!matchesRuntimeType(value, call.output)) throw new Error(`transform result is not assignable to ${typeString(call.output)}`);
    } catch (error) {
...
async function applyDeclared(state: RuntimeState, symbolId: string, input: JsonValue, path: string): Promise<JsonValue> {
  ...
  try {
    ...
    if (implementation === undefined) throw new ExecutionError("E_EXTERNAL_TRANSFORM_MISSING", `external transform ${JSON.stringify(transform.symbol)} is not registered`, { path });
    const context = state.signal === undefined ? {} : { signal: state.signal };
    const result = await implementation(context, input);
    if (!isJSONCompatible(result) || !matchesRuntimeType(result, transform.output)) throw new Error(`external transform result is not assignable to ${typeString(transform.output)}`);
    return result;
  } finally { state.callStack.delete(symbolId); }
}
...
function applyBuiltin(name: string, input: JsonValue, call: ResolvedTransformCallIR): JsonValue {
  ...
  if (name === "replace") return replaceCount(requireString(input), requiredString(arguments_, "old"), requiredString(arguments_, "new"), optionalInteger(arguments_, "count", -1));
  if (name === "regex-replace") return regexReplace(requireString(input), portableRegex(arguments_), requiredString(arguments_, "replacement"), optionalInteger(arguments_, "count", -1));
  if (name === "regex-capture") {
    ...
    const match = portableRegex(arguments_).exec(requireString(input));
  ...
  if (name === "split") {
    const value = requireString(input); const separator = requiredString(arguments_, "separator"); const limit = optionalInteger(arguments_, "limit", -1);
  ...
  if (name === "parse-int") return parseInteger(requireString(input), requiredString(arguments_, "as"), optionalInteger(arguments_, "radix", 10));
  ...
  if (name === "empty-to-null") { const value = requireString(input); return value === "" ? null : value; }
  ...
  if (name === "url-query") return new URL(requireString(input)).searchParams.getAll(requiredString(arguments_, "name"))[optionalInteger(arguments_, "index", 0)] ?? null;
  if (name === "url-path") return decodeURIComponent(new URL(requireString(input)).pathname);
  if (name === "path-segment") {
    const value = requireString(input); let path: string;
    try { path = new URL(value).pathname; } catch { path = value; }
    const parts = path.split("/").filter(Boolean).map(decodeURIComponent);
    let index = requiredInteger(arguments_, "index"); if (index < 0) index += parts.length;
    return parts[index] ?? null;
  }
  if (name === "assert-matches") { if (!portableRegex(arguments_).test(requireString(input))) throw new Error("value does not match required pattern"); return input; }
  if (name === "assert-enum") { if (!call.positionalArguments.some((value) => scalarEqual(value, input))) throw new Error("value is outside the allowed enum"); return input; }
  if (name === "assert-min" || name === "assert-max") {
    if (typeof input !== "number") throw new Error("expected number");
    const bound = requiredArgument(arguments_, "value"); if (typeof bound !== "number") throw new Error("numeric bound is required");
...
function requireString(value: JsonValue): string { if (typeof value !== "string") throw new Error(`expected string, got ${jsonType(value)}`); return value; }
function requiredArgument(arguments_: Readonly<Record<string, JsonScalar>>, name: string): JsonScalar { const value = arguments_[name]; if (value === undefined) throw new Error(`argument ${JSON.stringify(name)} is required`); return value; }
function requiredString(arguments_: Readonly<Record<string, JsonScalar>>, name: string): string { const value = requiredArgument(arguments_, name); if (typeof value !== "string") throw new Error(`argument ${JSON.stringify(name)} must be a string`); return value; }
function requiredInteger(arguments_: Readonly<Record<string, JsonScalar>>, name: string): number { const value = requiredArgument(arguments_, name); if (typeof value !== "number" || !Number.isInteger(value)) throw new Error(`argument ${JSON.stringify(name)} must be an integer`); return value; }
function optionalInteger(arguments_: Readonly<Record<string, JsonScalar>>, name: string, fallback: number): number { return arguments_[name] === undefined ? fallback : requiredInteger(arguments_, name); }

function replaceCount(value: string, oldValue: string, newValue: string, count: number): string {
  if (count === 0) return value;
  if (oldValue === "") {
    const characters = Array.from(value); let output = ""; let replacements = 0;
    for (let index = 0; index <= characters.length; index++) {
      if (count < 0 || replacements < count) { output += newValue; replacements++; }
      if (index < characters.length) output += characters[index];
    }
    return output;
  }
  if (count < 0) return value.split(oldValue).join(newValue);
  let output = value; let start = 0;
  for (let index = 0; index < count; index++) {
    const found = output.indexOf(oldValue, start); if (found < 0) break;
    output = output.slice(0, found) + newValue + output.slice(found + oldValue.length); start = found + newValue.length;
  }
  return output;
}

function portableRegex(arguments_: Readonly<Record<string, JsonScalar>>): RegExp {
  const pattern = requiredString(arguments_, "pattern"); const flags = typeof arguments_.flags === "string" ? arguments_.flags : "";
  const seen = new Set<string>();
  for (const flag of flags) {
    if (!["i", "m", "s"].includes(flag)) throw new Error(`unsupported regex flag ${JSON.stringify(flag)}`);
    if (seen.has(flag)) throw new Error(`duplicate regex flag ${JSON.stringify(flag)}`);
    seen.add(flag);
  }
  if (/\(\?(?:P?<|[=!]|<[=!])|\\[1-9]/u.test(pattern)) throw new Error("regex is outside the portable RE2 profile");
  return new RegExp(pattern, `${flags}u`);
}

function regexReplace(value: string, expression: RegExp, replacement: string, count: number): string {
  if (count === 0) return value;
  const flags = expression.flags.includes("g") ? expression.flags : `${expression.flags}g`;
  const global = new RegExp(expression.source, flags); let replacements = 0;
  return value.replace(global, (...arguments_: unknown[]) => replacements++ < count || count < 0 ? expandReplacement(replacement, arguments_) : String(arguments_[0]));
}

function expandReplacement(replacement: string, matchArguments: readonly unknown[]): string {
  return replacement.replace(/\$(\$|\d{1,2})/gu, (_all, token: string) => token === "$" ? "$" : String(matchArguments[Number(token)] ?? ""));
}

function normalizeIndex(index: number, length: number): number { return Math.max(0, Math.min(length, index < 0 ? length + index : index)); }
...
function digitValue(character: string): number { const code = character.codePointAt(0)!; if (code >= 48 && code <= 57) return code - 48; if (code >= 97 && code <= 122) return code - 87; return -1; }

function parseFloatValue(value: string, type: string): number {
  if (!/^[+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?$/u.test(value)) throw new Error(`invalid finite decimal float ${JSON.stringify(value)}`);
  let output = Number(value); if (!Number.isFinite(output)) throw new Error(`parse ${type}: value out of range`);
  if (type === "f32") output = Math.fround(output); else if (type !== "float" && type !== "f64") throw new Error(`unsupported float target ${JSON.stringify(type)}`);
  if (!Number.isFinite(output)) throw new Error(`parse ${type}: value out of range`); return output;
}
...
function parseBoolean(value: string, arguments_: Readonly<Record<string, JsonScalar>>): boolean {
  ...
  let compared = value;
  if (!sensitive) { compared = compared.toLowerCase(); trueValue = trueValue.toLowerCase(); falseValue = falseValue.toLowerCase(); }
  if (compared === trueValue) return true; if (compared === falseValue) return false;
...
function matchesRuntimeType(value: JsonValue, type: TypeRef): boolean {
  ...
  const ranges: Readonly<Record<string, readonly [number, number]>> = { i8: [-128, 127], u8: [0, 255], i16: [-32768, 32767], u16: [0, 65535], i32: [-2147483648, 2147483647], u32: [0, 4294967295] };
...
function isJSONCompatible(value: unknown): value is JsonValue {
  ...
  if (typeof value === "object") return Object.getPrototypeOf(value) === Object.prototype && Object.values(value).every(isJSONCompatible);
  return false;
}

function scalarEqual(left: JsonScalar, right: JsonValue): boolean { return typeof right !== "object" && Object.is(left, right); }

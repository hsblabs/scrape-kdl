...
function validate(schema, value, path, root, errors) {
  ...
  if (schema.enum !== undefined && !schema.enum.some((candidate) => isDeepStrictEqual(value, candidate)))
    errors.push(`${path}: value is outside the enum`);
  ...
  if (typeof value === "number") {
    ...
    if (schema.maximum !== undefined && value > schema.maximum)
      errors.push(`${path}: number exceeds maximum ${schema.maximum}`);
  }
  ...
  if (Array.isArray(value)) {
    ...
    if (schema.items !== undefined)
      value.forEach((item, index) => validate(schema.items, item, `${path}[${index}]`, root, errors));
  }
    ...
    for (const [name, dependencies] of Object.entries(schema.dependentRequired ?? {})) {
      if (name in value)
        for (const dependency of dependencies)
          if (!(dependency in value)) errors.push(`${path}.${dependency}: property is required by ${name}`);
    }
  }
}

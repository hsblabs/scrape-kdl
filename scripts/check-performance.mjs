...
const extractionWorkload = async () => {
  await prepared.program.extract(inputs, { fetch: async () => new Response(html, { headers: { "content-type": "text/html; charset=utf-8" } }) });
};
const typescript = {
  compileRatio: await ratio(compileWorkload, async () => { JSON.parse(irText); }),
  extractRatio: await ratio(extractionWorkload, async () => { JSON.parse(outputText); }),
};
const go = JSON.parse(execFileSync("go", ["run", "./scripts/performanceprobe"], {
  cwd: root, encoding: "utf8", env: { ...process.env, GOTOOLCHAIN: "local" }, stdio: ["ignore", "pipe", "inherit"],
}));
...
if (measureOnly) {
  ...
  for (const runtime of ["go", "typescript"]) {
    for (const workload of ["compileRatio", "extractRatio"]) {
      ...
      assert.ok(actual <= gate.maximum, `${runtime}.${workload} ${actual.toFixed(3)} exceeds measured baseline ${gate.baseline} and maximum ${gate.maximum}`);
      console.log(`performance: ${runtime}.${workload}=${actual.toFixed(3)} (baseline ${gate.baseline}, maximum ${gate.maximum})`);
    }
  }
}

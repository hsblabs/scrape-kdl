...
const corePackageJSON = JSON.parse(await readFile(join(root, "packages/scrape-kdl/package.json"), "utf8"));
...
assert.equal(matrix.minimums.node, "22");
assert.equal(matrix.minimums.bun, "1.3");
assert.equal(packageJSON.engines.node, ">=22");
assert.equal(packageJSON.engines.bun, ">=1.3");
assert.equal(corePackageJSON.engines.node, ">=22");
assert.equal(corePackageJSON.engines.bun, ">=1.3");
